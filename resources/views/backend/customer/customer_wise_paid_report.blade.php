...
@section('admin-content')
 ...
 <div class="page-content print-container">
    ...
    <div class="row">
        <div class="col-12">
            <div class="invoice-title">
                <h4 class="float-end font-size-16"><strong>Invoice No # {{ $payment->invoice_id ?? '0'}}</strong></h4>
                <h3>
                ...
                <div class="col-6 mt-4 text-end">
                    <address>
                        <strong>Invoice Date:</strong><br>
                         {{ date('d-m-Y',strtotime($payment->invoice->date ?? Carbon\Carbon::now()))  }} <br><br>
                    </address>
                </div>
            </div>
        </div>
    ...
    @php
    // dd($payment->invoice->id ?? $payment->invoice_id);
    ...
    <table class="table">
        ...
        <tr>
            ...
            <td class="text-center">{{ $payment['customer']['email'] ?? '' }}</td>
             <td class="text-center">{{ $payment->invoice->description ?? '' }}</td>
...
@php
    $paymentDetails = App\Models\PaymentDetail::where('invoice_id',$payment->invoice_id)->get();
    // dd($paymentDetails);
@endphp
            ...
            <tr>
                <td colspan="4" style="text-align: center;font-weight:bold">{{ $item->date }}</td>
